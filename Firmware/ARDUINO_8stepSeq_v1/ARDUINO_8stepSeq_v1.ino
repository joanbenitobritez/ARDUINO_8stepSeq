...
bool ignorarLiberacion[8] = { false, false, false, false, false, false, false, false };  // FLAG DE CONTROL PARA EVITAR EL CAMBIO DE ESTADO DE PASO EN MODO COMBO
...
void loop() {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (estadoActual == LOW && estadoAnterior[i] == HIGH)  // DETECTO EL FLANCO DE BAJADA DE CADA BOTON
    {
      tiempoRebote[i] = millis();
      botonesPresionados++;  // INCREMENTO EL CONTADOR DE BOTONES PRESIONADOS EN CADA BAJADA
    ...
    if (estadoActual == LOW && estadoAnterior[i] == LOW && millis() - tiempoRebote[i] > 300)  // SI EL TIEMPO DE RETENCION SUPERA LOS 300MS, ENTRAMOS EN MODO NOTA PARA ESE PASO
    {
      if (ignorarLiberacion[i] == false)  // SI NO IGNORA LA LIBERACION, PERMITE AFINAR LA NOTA
      {
        ...
        digitalWrite(pinLeds[i], (millis() / 50) % 2);  // PARPADEO LED
