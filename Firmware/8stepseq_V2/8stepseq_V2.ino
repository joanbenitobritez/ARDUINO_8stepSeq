...
void loop() {
  ...
  if ((btnStepUp.fell() && dnActivo) || (stepDnPresionado && upActivo)) {
    ...
    return;  // Evita propagar la lectura accidental a los pasos inferiores
  }
  ...
  if (!modoTeclado) {
    ...
    long nuevaPosicion = miEncoder.read() / RESOLUCION_ENCODER;
    int delta = nuevaPosicion - posicionEncoderAnt;

    if (delta != 0) {
      posicionEncoderAnt = nuevaPosicion;
  ...
  if (!modoTeclado) {
    ...
    if (refrescarTextosNotas) {
      u8x8.setCursor(0, 6);
      u8x8.print("EDIT P");
      if (pasoEditadoActual != -1) {
        u8x8.print(pasoEditadoActual);
        u8x8.print(": ");
        u8x8.print(nombresNotas[notaMostradaEdit]);
      } else {
        u8x8.print("-: -- ");
      }
      u8x8.print("   ");
      ...
      u8x8.setCursor(0, 7);
      u8x8.print("PLAY: ");
      if (notaMostradaPlay != -1) u8x8.print(nombresNotas[notaMostradaPlay]);
      else u8x8.print("-- ");
      u8x8.print("   ");
    ...
    u8x8.setCursor(0, 3);
    u8x8.print("OCTAVA: ");
    if (octavaActual > 0) u8x8.print("+");
    u8x8.print(octavaActual);
    u8x8.print("  ");
